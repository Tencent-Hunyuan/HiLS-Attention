...
def _decode_and_inspect(tokenizer, micro_batch, batch_idx, chunk_size=None, observe_seq_len=8192):
    ...
    logger.info_rank0(f"\n{'='*80}")
    logger.info_rank0(f"[Batch {batch_idx}] keys={list(micro_batch.keys())}")
    for k, v in micro_batch.items():
        if isinstance(v, torch.Tensor):
            logger.info_rank0(f"  {k}: shape={v.shape}, dtype={v.dtype}")
        else:
            logger.info_rank0(f"  {k}: {type(v).__name__}")
    ...
    logger.info_rank0(f"\n  seq_len={total_tokens}, masked(-100)={masked_count}, trainable={train_count} ({100*train_count/total_tokens:.1f}%)")

    decoded_input = tokenizer.decode(ids.tolist(), skip_special_tokens=False)
    logger.info_rank0(f"\n  [INPUT (first 500 chars)]:")
    logger.info_rank0(f"  {decoded_input[:500]}")
    ...
    logger.info_rank0(f"  {decoded_train[:500]}")
    ...
    for pos, direction, text, lab_window in transitions[:20]:
        lab_str = ["T" if l != -100 else "M" for l in lab_window]
        logger.info_rank0(f"    pos={pos:>6d} {direction:<12s} labels=[{','.join(lab_str)}] text='{text}'")
    if len(transitions) > 20:
        logger.info_rank0(f"    ... and {len(transitions) - 20} more transitions")
    ...
    if chunk_size and chunk_size > 0:
        ...
        if "position_ids" in micro_batch:
            ...
            logger.info_rank0(f"    position_ids resets at: {reset_positions[:30]}")

    logger.info_rank0(f"\n  [TOKEN-LEVEL last {observe_seq_len} tokens]:")
    logger.info_rank0(f"  {'pos':>5s} | {'id':>8s} | {'lab':>8s} | {'mask':>4s} | token_text")
    logger.info_rank0(f"  {'-'*5}-+-{'-'*8}-+-{'-'*8}-+-{'-'*4}-+-{'-'*30}")
    for i in range(0, seq_len):
        ...
        logger.info_rank0(f"  {i:>5d} | {tok_id:>8d} | {lab_val:>8d} | {mask_str:>4s} | {tok_text}")

    logger.info_rank0(f"{'='*80}\n")
# ==================== END DEBUG ====================
...
def main():
    ...
    # Build collator pipeline: default packing + pad to fixed length
    from veomni.data.data_collator import DataCollatorWithPositionIDs
    collate_fn = [
        DataCollatorWithPositionIDs(),
        PadToFixedLengthCollator(target_length=args.data.max_seq_len),
    ]
    ...
    for epoch in range(start_epoch, args.train.num_train_epochs):
        ...
        for _ in range(start_step, args.train.train_steps):
            ...
            if global_step == 1:
                _decode_and_inspect(tokenizer, micro_batches[0], batch_idx=global_step, chunk_size=args.train.chunk_size)
            # ==================== END DEBUG ====================
