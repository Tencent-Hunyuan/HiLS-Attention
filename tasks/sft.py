...
from utils.arguments import CPTArguments
...
@dataclass
class SFTDataArguments(DataArguments):
    data_type: str = field(
    ...
    chunk_size: int = field(
        default=64,
        metadata={"help": "Landmark chunk size for HSA."},
    )
...
class Arguments:
    ...
    data: "DataArguments" = field(default_factory=SFTDataArguments)
    train: "TrainingArguments" = field(default_factory=CPTArguments)


def insert_lmk_for_sft(
    example,
    chat_template,
    max_seq_len: int,
    chunk_size: int,
    ...
    """
    Wrap process_sft_example to insert landmark tokens into each SFT sample.

    Steps:
    1. Tokenize with process_sft_example at a reduced max_seq_len so that
       after lmk insertion the total length fits within max_seq_len.
    2. Insert lmk tokens into input_ids, attention_mask, and labels.
    3. Generate position_ids with landmark positions.
    4. Return dict with position_ids so DataCollatorWithPositionIDs can pack correctly.
    """
    # After inserting lmk tokens, every (chunk_size-1) original tokens become chunk_size tokens.
    # So we need to reduce the max_seq_len to leave room for the inserted lmk tokens.
    pre_lmk_max_seq_len = max_seq_len * (chunk_size - 1) // chunk_size

    # Get tokenized SFT sample from veomni
    samples = process_sft_example(
        example,
        chat_template=chat_template,
        max_seq_len=pre_lmk_max_seq_len,
        text_keys=text_keys,
    )

    results = []
    for sample in samples:
        input_ids = sample["input_ids"]       # (L,)
        attention_mask = sample["attention_mask"]  # (L,)
        labels = sample["labels"]             # (L,)

        # Ensure tensors and add batch dim for insert_special_tokens (expects [N, L])
        ...
        # Insert lmk token into input_ids
        input_ids = insert_special_tokens(input_ids, lmk_id, chunk_size)  # (1, L')

        # Insert 1 into attention_mask at lmk positions
        attention_mask = insert_special_tokens(attention_mask, 1, chunk_size)  # (1, L')

        # Insert -100 into labels at lmk positions
        # Match the model forward logic: shift -> insert -> unshift
        labels = torch.roll(labels, shifts=-1, dims=-1)
        labels[:, -1] = -100
        labels = insert_special_tokens(labels, -100, chunk_size)
        labels = torch.roll(labels, shifts=1, dims=-1)

        # Generate position_ids with landmark positions
        position_ids = create_position_ids_with_landmarks(L, chunk_size, input_ids.device)  # (1, L')
...
def main():
    ...
    # lmk_id is conventionally vocab_size (the first token beyond the original vocabulary)
    ...
    # Disable model-level landmark insertion — dataloader already handles it
    model.insert_landmarks = False
    logger.info_rank0(f"Set model.insert_landmarks = False (lmk inserted in dataloader, chunk_size={args.data.chunk_size})")
    ...
    if args.train.global_rank == 0:
        ...
        # save model_assets before training — save chat_template for SFT (not tokenizer)
