...
class Qwen2Attention(nn.Module):
    ...
    def __init__(self, config: HSASWAConfig, layer_idx: int):
        super().__init__()
        self.config = config
        ...
        self.enable_stable_enc = getattr(config, 'enable_stable_enc', False)
        if self.enable_stable_enc:
...
class Qwen2DecoderLayer(nn.Module):
    def __init__(self, config: HSASWAConfig, layer_idx: int):
        ...
        self.encoder_dropout = getattr(config, 'encoder_dropout', 0.0)
    ...
    def forward(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        hidden_states = self.mlp(F.dropout(hidden_states, p=self.encoder_dropout, training=self.training))
        hidden_states = residual + hidden_states
...
class Qwen2RotaryEmbedding(nn.Module):
    def __init__(self, config: HSASWAConfig, device=None):
        super().__init__()
