...
class HybridNSAAttention(nn.Module):
    def __init__(self, config, layer_idx):
        ...
        assert self.hsa_heads % self.hsa_qk_ratio == 0
        assert self.h_q % self.hsa_heads == 0
        self.hsa_denom = self.h_q // self.hsa_heads
        assert self.h_kv % self.hsa_denom == 0
        self.h_hsa_kv = self.hsa_heads // self.hsa_qk_ratio

        self.nsa_kernel_min_qk_ratio = 16  # Group size must be a multiple of 16 in NSA
        self.nsa_kernel_num_heads = max(self.hsa_heads, self.nsa_kernel_min_qk_ratio * self.h_hsa_kv)

        self.real_group_size = self.hsa_heads // self.h_hsa_kv
        self.padded_group_size = self.nsa_kernel_num_heads // self.h_hsa_kv
        ...
        self.g_proj = nn.Linear(self.d_model, self.hsa_heads * 3, bias=False)
        ...
        self.nsa_block_counts = getattr(config, 'nsa_block_counts', 32)
        self.nsa_window_size = getattr(config, 'nsa_window_size', 512)
    ...
    def forward(
        ...
        **kwargs,
        ...
        # NSA 
        nsa_q = self.hsa_q_proj(hidden_states)
        nsa_q = rearrange(nsa_q, 'B L (h d)->B L h d', d=self.head_dim)
        nsa_q = self.q_norm(nsa_q)

        nsa_k = self.hsa_k_proj(hidden_states)
        nsa_k = rearrange(nsa_k, 'B L (h d)->B L h d', d=self.head_dim)
        nsa_k = self.k_norm(nsa_k)

        nsa_v = self.hsa_v_proj(hidden_states)
        nsa_v = rearrange(nsa_v, 'B L (h d)->B L h d', d=self.head_dim)

        g = rearrange(self.g_proj(hidden_states), 'B L (h d) -> B L h d', d=3)
        g_cmp, g_slc, g_swa = g.sigmoid().unbind(-1)

        if self.pad_per_group > 0:
            nsa_q_padded = rearrange(nsa_q, 'B L (k g) d -> B L k g d', k=self.h_hsa_kv)
            nsa_q_padded = F.pad(nsa_q_padded, (0, 0, 0, self.pad_per_group), value=0.0)
            nsa_q_padded = rearrange(nsa_q_padded, 'B L k g d -> B L (k g) d')

            g_cmp_padded = rearrange(g_cmp, 'B L (k g) -> B L k g', k=self.h_hsa_kv)
            g_cmp_padded = F.pad(g_cmp_padded, (0, self.pad_per_group), value=0.0)
            g_cmp_padded = rearrange(g_cmp_padded, 'B L k g -> B L (k g)')

            g_slc_padded = rearrange(g_slc, 'B L (k g) -> B L k g', k=self.h_hsa_kv)
            g_slc_padded = F.pad(g_slc_padded, (0, self.pad_per_group), value=0.0)
            g_slc_padded = rearrange(g_slc_padded, 'B L k g -> B L (k g)')

            g_swa_padded = rearrange(g_swa, 'B L (k g) -> B L k g', k=self.h_hsa_kv)
            g_swa_padded = F.pad(g_swa_padded, (0, self.pad_per_group), value=0.0)
            g_swa_padded = rearrange(g_swa_padded, 'B L k g -> B L (k g)')
        else:
            nsa_q_padded = nsa_q
            g_cmp_padded = g_cmp
            g_slc_padded = g_slc
        ...
        if self.pad_per_group > 0:
            nsa_o = rearrange(nsa_o_padded, 'B L (k g) d -> B L k g d', k=self.h_hsa_kv)
            nsa_o = nsa_o[:, :, :, :self.real_group_size, :]
            nsa_o = rearrange(nsa_o, 'B L k g d -> B L (k g) d')
        else:
            nsa_o = nsa_o_padded

        o = torch.cat([swa_o, nsa_o], dim=2)
...
class HybridNSAModel(Qwen3PreTrainedModel):
    """
...
class HybridNSAForCausalLM(Qwen3PreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config):
        super().__init__(config)
        self.model = HybridNSAModel(config)
        self.vocab_size = next_of_y(config.vocab_size + 1, 32)
...
__all__ = ["HybridNSAForCausalLM", "HybridNSAModel", "Qwen3PreTrainedModel"]
