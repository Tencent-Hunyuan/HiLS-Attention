...
from utils.flex_attn import flex_attn, cu_seqlens_to_doc_ids
...
class LandmarkHSA(nn.Module):
    def __init__(self, config, layer_idx, norm_cls=None):
        ...
        from ops.hsa_fwd_bwd_group_forbench import HSA_block_M_group as HSA
        from ops.topk_group import online_topk_group as topk_func
        from ops.rope_tilelang_fp32 import hsa_intra_chunk_rope_autograd as hsa_intra_chunk_rope
        ...
        self.force_recent_chunks = getattr(config, "force_recent_chunks", 0)

        # ── bench 可调 kernel 参数（外部通过 set_hsa_kernel_params 设置）──
        self.bench_block_M_fwd = None
        self.bench_block_M_bwd = None
        self.bench_num_threads_fwd = None
        self.bench_num_threads_bwd = None

        # print("force_recent_chunks:", self.force_recent_chunks)
    ...
    def forward(self,
        ...
        **kwargs,
        ...
        # print(f"Layer {self.layer_idx} input shape: {hidden_states.shape}")
        ...
        if use_cache and past_key_values is not None:
            ...
            hsa_k_norm, hsa_v = past_key_values.update(
                hsa_k_norm, hsa_v, hsa_cache_idx, cache_kwargs
            )
        ...
        if self.enable_hsa_swa:
            # chunk prefill 时 hsa_swa_k_norm 仍指向当前 segment，
            # 而 hsa_v 已经是完整序列，导致 K/V seq_len 不匹配。
            # 使用 cache update 后的 hsa_k_norm 保证 K/V 长度一致。
            swa_k = hsa_k_norm if use_cache else hsa_swa_k_norm
        ...
        if full_seq_len >= self.chunk_size:
            ...
            if self.apply_hsa_rope:
                q_cos_chunk = cos[:, self.chunk_size + self.hsa_sliding_window : 2 * self.chunk_size + self.hsa_sliding_window, :].contiguous()
                q_sin_chunk = sin[:, self.chunk_size + self.hsa_sliding_window : 2 * self.chunk_size + self.hsa_sliding_window, :].contiguous()
                k_cos_chunk = cos[:, :self.chunk_size, :].contiguous()
                k_sin_chunk = sin[:, :self.chunk_size, :].contiguous()
                hsa_q_norm, hsa_k_norm = self.hsa_intra_chunk_rope(
                    hsa_q_norm, hsa_k_norm,
                    q_cos_chunk, q_sin_chunk,
                    k_cos_chunk, k_sin_chunk,
                ) 

            # print(f"[DEBUG lhsa_layer] layer={self.layer_idx} bench_block_M_fwd={self.bench_block_M_fwd}, bench_num_threads_fwd={self.bench_num_threads_fwd}")
            hsa_o = self.hsa_func(
                hsa_q_norm, hsa_k_norm, hsa_v,
                weights=chunk_weights, indices=indices,
                block_size=self.chunk_size, mask_last_token=True,
                block_M_fwd=self.bench_block_M_fwd,
                block_M_bwd=self.bench_block_M_bwd,
                num_threads_fwd=self.bench_num_threads_fwd,
                num_threads_bwd=self.bench_num_threads_bwd,
                is_training=self.training,
            )
            if self.enable_hsa_swa:
                swa_o_weight = chunk_weights[:, :, :, swa_weight_idx]  # (B, L, h_kv)
                # o = hsa_o + swa_o * swa_o_weight.unsqueeze(-1)
                swa_o_weight_expanded = swa_o_weight.repeat_interleave(self.hsa_qk_ratio, dim=2)
