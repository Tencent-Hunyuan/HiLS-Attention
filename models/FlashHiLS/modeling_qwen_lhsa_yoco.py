...
import torch
import math
from .hsa_forward import hsa_causal_lm_forward
from torch import nn
from .lhsa_layer_share import LandmarkHSA
...
try:
    ...
    logger.warning_once(
        "USE_FLASH_ATTN_RMSNORM=1 but flash-attn is not available; falling back to torch RMSNorm."
    )
...
class HSAModel(Qwen3PreTrainedModel):
    ...
    def __init__(self, config: Qwen3Config):
        ...
        self.shared_k_proj = nn.Linear(self.d_model, self.h_hsa_kv * self.head_dim, bias=False)
        self.shared_v_proj = nn.Linear(self.d_model, self.h_hsa_kv * self.head_dim, bias=False)
        self.k_norm = Qwen3RMSNorm(self.head_dim, eps=config.rms_norm_eps)

        # self.forward = partial(hsa_model_forward, self)
    ...
    def forward(
        ...
        **flash_attn_kwargs,
        ...
        for decoder_layer in self.layers[: self.config.num_hidden_layers // 2]:
        ...
        shared_k = self.shared_k_proj(hidden_states)
        shared_v = self.shared_v_proj(hidden_states)
        shared_k = rearrange(shared_k, 'b l (h d) -> b l h d', d=self.head_dim)
        shared_v = rearrange(shared_v, 'b l (h d) -> b l h d', d=self.head_dim)
        shared_k = self.k_norm(shared_k)
...
class HSAForCausalLM(Qwen3PreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config):
        ...
        self.forward = partial(hsa_causal_lm_forward, self)
        # Initialize weights and apply final processing
        self.post_init()
