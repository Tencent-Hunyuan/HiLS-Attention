"""MATH 4-shot generation config.

NOTE on braces: OpenCompass's PromptTemplate uses `safe_format` which does
a plain `str.replace(f'{{{k}}}', v)` — it does NOT un-escape `{{` → `{`.
The upstream configs/datasets/math/math_gen_a58d9d.py has a historical
bug where LaTeX braces were written as `{{...}}` (legacy from when
`str.format` was used). With safe_format, those double braces bleed into
the prompt verbatim, heavily polluting the few-shot examples.

We fix this by writing single-brace LaTeX here.
"""
...
math_eval_cfg = dict(
    ...
    pred_postprocessor=dict(type='math_unbrace'),
)
